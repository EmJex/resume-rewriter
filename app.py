...
def call_claude(api_key: str, system: str, user_message: str, max_tokens: int = 4096) -> str:
    """调用Claude API"""
...
if rewrite_btn and resume_text and jd_text and api_key:
    ...
    with st.status("🔍 分析岗位匹配度...", expanded=True) as status:
        try:
            ...
            analysis = call_claude(api_key, ANALYSIS_PROMPT, analysis_input, max_tokens=1024)
    ...
    with st.status("✍️ 正在改写简历...", expanded=False) as status:
        try:
            ...
            rewritten = call_claude(api_key, SYSTEM_PROMPT, rewrite_input)
